...
impl CustomPdf {
    ...
    // Sample PDF maker of creating Reciept.
    // #[allow(unused_variables)]
    // pub fn create_reciept_pdf_printpdf(&self) {
    //     const WIDTH: f64 = 297.0;
    //     const HEIGHT: f64 = 210.0;

    //     const TITLE: &str = "受領書";
    //     let (doc, page1, layer1) = PdfDocument::new("Reciept", Mm(297.0), Mm(210.0), "Layer 1");

    //     let current_layer = doc.get_page(page1).get_layer(layer1);

    //     let maru_gothic = doc.add_external_font(&self.fontfile).unwrap();

    //     let black       = Color::Rgb(Rgb::new(0.1, 0.1, 0.1, None));
    //     let yellow      = Color::Cmyk(Cmyk::new(0.0, 0.0, 1.0, 0.0, None));
    //     let grey        = Color::Cmyk(Cmyk::new(0.0, 0.0, 0.0, 0.5, None));
    //     let text_white  = Color::Cmyk(Cmyk::new(0.0, 0.0, 0.0, 0.03, None));

    //     let title_box_points: Vec<(Point, bool)> =
    ...
    //     // Rounded Rect is created by rounded outline.
    ...
    //     current_layer.add_shape(title_box);

    //     current_layer.set_fill_color(text_white);
    //     current_layer.set_character_spacing(10.0);
    //     current_layer.use_text(TITLE, 24.0, Mm(WIDTH / 2.0 - 16.0), Mm(HEIGHT - 18.0), &maru_gothic);

    //     doc.save(&mut BufWriter::new(File::create("target/tmp.pdf").unwrap())).unwrap();
    // }
}
